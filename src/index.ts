...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DBpedia');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'SPARQL query text. Also accepts `sparql` as an alias — the wikidata_sparql_query tool uses that name for the same argument.' },
        sparql: { type: 'string', description: 'Alias for `query`.' },
        format: { type: 'string', description: 'json (default) | xml | csv | tsv' },
      },
      // Deliberately not `required: ['query']` — the gateway's pre-flight
      // guard rejects a call missing a declared required argument BEFORE this
      // pack ever runs, so a caller sending only the `sparql` alias would
      // never reach the runtime check below. See reqSparqlQuery().
    },
  },
  {
    name: 'resource',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'lookup': {
      ...
      const res = await pwFetch(`${LOOKUP}?${params}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'DBpedia Lookup');
      return parseJson(res, 'DBpedia Lookup');
    }
    case 'sparql': {
      ...
      const params = new URLSearchParams({
        query: reqSparqlQuery(args),
        format: accept[format] ?? accept.json,
      });
      const res = await pwFetch(`${SPARQL}?${params}`, {
      ...
      // DBpedia's 503 is a Virtuoso HTML page — pasting it handed the caller
      // `<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML//EN">…` (fleet #712).
      if (!res.ok) throw await httpError(res, 'DBpedia SPARQL');
      if ((accept[format] ?? accept.json) === accept.json) return parseJson(res, 'DBpedia SPARQL');
    ...
    case 'resource': {
      ...
      const res = await pwFetch(`${RES}/${encodeURIComponent(label)}.json`, {
        headers: { Accept: 'application/json', 'User-Agent': UA },
      });
      if (res.status === 404) throw new Error('DBpedia: resource not found');
      if (!res.ok) throw await httpError(res, 'DBpedia');
      return parseJson(res, 'DBpedia');
    }
    case 'abstract': {
      ...
      if (!wikiRes.ok) throw await httpError(wikiRes, 'Wikipedia (abstract fallback)');
      const data = (await parseJson<{ extract?: string; title?: string; content_urls?: { desktop?: { page?: string } } }>(wikiRes, 'Wikipedia (abstract fallback)')) as { extract?: string; title?: string; content_urls?: { desktop?: { page?: string } } };
      return {
...
/**
 * `query` is the declared argument name for the `sparql` tool, but wikidata's
 * equivalent tool uses `sparql` for the same concept — the mirror-image of
 * this pack's naming. Accept either; name both in the error when neither is
 * given. Fleet #675.
 */
function reqSparqlQuery(args: Record<string, unknown>): string {
  const v = (args.query ?? args.sparql) as unknown;
  if (typeof v !== 'string' || !v.trim()) {
    throw new Error(
      'Required argument "query" (alias "sparql") is missing. Pass a string like ' +
        '"SELECT ?p WHERE { ?p a <http://dbpedia.org/ontology/Person> } LIMIT 5" as either `query` or `sparql`.',
    );
  }
  return v;
}
